...
public class JsonMapCarrier implements MapCarrier {
    ...
    public <T> T getWithErrors(@NotNull KeyedEndec<T> key) {
        return this.object.containsKey(key.key()) ? key.endec().decodeFully(JsonDeserializer::of, this.object.get(key.key())) : key.defaultValue();
    }

    @Override
    public <T> void put(@NotNull KeyedEndec<T> key, @NotNull T value) {
        this.object.put(key.key(), key.endec().encodeFully(JsonSerializer::of, value));
    }
